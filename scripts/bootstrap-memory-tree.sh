...
[ -L "$KNOWLEDGE_POINTER_PATH" ] || kc_fail "Bootstrap requires ./agent-knowledge to already be a pointer to the external knowledge folder. Run scripts/install-project-links.sh first."
...
if [ "$JSON_MODE" -ne 1 ]; then
    ...
    kc_log "Next steps:"
    kc_log "  1. Collect evidence: scripts/import-agent-history.sh $TARGET_PROJECT"
    kc_log "  2. Run validation:  scripts/validate-knowledge.sh $TARGET_PROJECT"
